...
export const DialogComponent: React.FC<
  DialogOptions & { onResolve: (value: boolean) => void; close: () => void }
> = ({
  type,
  title,
  message,
  ...
  return (
    <div
      className={`fixed inset-0 z-[9999] flex items-center justify-center p-4 transition-opacity duration-200 ${isOpen ? 'opacity-100' : 'opacity-0'}`}
    >
      <div
        className="absolute inset-0 bg-slate-900/40 backdrop-blur-sm"
        onClick={() => handleClose(type !== 'confirm')}
      ></div>
      <div
        className={`bg-white rounded-3xl p-6 w-full max-w-sm shadow-2xl relative z-10 transition-all duration-200 ${isOpen ? 'scale-100 translate-y-0' : 'scale-95 translate-y-4'}`}
      >
        ...
        <div className="py-2 text-slate-600 mb-6 font-medium">{message}</div>
