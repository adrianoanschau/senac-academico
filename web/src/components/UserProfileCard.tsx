...
export const UserProfileCard: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm flex flex-col items-center text-center">
      ...
      {profile?.roles && profile.roles.length > 0 && (
        <div className="flex flex-wrap items-center justify-center gap-1.5 mt-3">
          {profile.roles.map((role) => (
            <span
              key={role}
              className={`px-2 py-0.5 rounded-full text-[10px] font-bold border capitalize ${roleColors[role] || roleColors[Role.MEMBER]}`}
            >
              {role.toLowerCase()}
            </span>
          ))}
      ...
      <Link
        to="/settings"
        className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold text-slate-600 rounded-lg hover:bg-slate-100 transition-colors"
      >
        <Settings size={16} />
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold text-rose-600 rounded-lg hover:bg-rose-50 transition-colors"
      >
        <LogOut size={16} />
