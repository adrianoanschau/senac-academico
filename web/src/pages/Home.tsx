    ...
    const fetchData = async () => {
      try {
        ...
        const classGroupsList =
          classGroupsRes.data.data || classGroupsRes.data || [];
        const professorsList =
          professorsRes.data.data || professorsRes.data || [];
        const roomsList = roomsRes.data.data || roomsRes.data || [];
        const curriculumsList =
          curriculumsRes.data.data || curriculumsRes.data || [];
        ...
        classGroupsList.forEach((cg: ClassGroup) => {
          ...
          if (cg.curriculumId) {
            const curriculum = curriculumsList.find(
              (c: Curriculum) => c.id === cg.curriculumId,
            );
            if (curriculum && curriculum.course) {
              courseName = curriculum.course.name;
            } else if (curriculum) {
              const course = coursesList.find(
                (co: Course) => co.id === curriculum.courseId,
              );
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm mt-4">
          ...
          <div className="flex justify-between items-center text-xs text-slate-600 mb-2">
            <div className="flex items-center gap-1.5">
              <div className="w-2 h-2 rounded-full bg-emerald-500"></div> Banco
              de Dados
            </div>
