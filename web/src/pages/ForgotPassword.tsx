    ...
    try {
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : 'Falha ao processar solicitação. Tente novamente.';
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-senac-blue relative overflow-hidden flex-col justify-center items-center p-12 transition-colors">
        ...
        <div className="relative z-10 max-w-lg">
          ...
          <p className="text-xl text-blue-100 font-medium leading-relaxed">
            Recupere o acesso à plataforma para continuar gerenciando sua rotina
            acadêmica.
          </p>
        </div>
      </div>
        ...
        <div className="w-full max-w-md bg-white rounded-4xl p-8 sm:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.03)] border border-slate-100">
          ...
          {success ? (
            <div className="flex flex-col items-center justify-center p-6 bg-emerald-50 border border-emerald-100 rounded-2xl text-center">
              ...
              <Link
                to="/login"
                className="w-full bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-3 px-4 rounded-xl transition-colors flex items-center justify-center"
              >
                Voltar ao login
              </Link>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="w-full bg-senac-blue hover:opacity-90 disabled:opacity-70 disabled:cursor-not-allowed text-white font-bold py-4 px-4 rounded-xl transition-all mt-4 flex items-center justify-center gap-2 shadow-md shadow-senac-blue/30 hover:shadow-lg hover:shadow-senac-blue/40"
              >
                {isSubmitting ? (
              ...
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-3.5 px-4 font-semibold text-slate-500 hover:text-slate-800 bg-transparent transition-colors"
              >
                <ArrowLeft size={18} />
                Voltar para o login
              </button>
