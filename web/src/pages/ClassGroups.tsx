...
export const ClassGroups: React.FC = () => {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEditing ? `/api/class-groups/${formData.id}` : '/api/class-groups';
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-100">
                ...
                <th className="py-4 px-4 font-bold text-slate-400 text-sm">Grade Curricular</th>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr key={turma.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="py-4 px-4 text-slate-500 font-medium">{formatDate(turma.startDate)}</td>
                  <td className="py-4 px-4 text-slate-500 font-medium">{formatDate(turma.endDate)}</td>
                  <td className="py-4 px-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                      turma.shift === 'Manhã' ? 'bg-amber-100 text-amber-700' :
                      turma.shift === 'Tarde' ? 'bg-orange-100 text-orange-700' : 'bg-indigo-100 text-indigo-700'
                    }`}>
                      {turma.shift}
                    </span>
                  </td>
