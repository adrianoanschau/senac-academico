...
export const Curriculums: React.FC = () => {
  ...
  const fetchDependencies = async () => {
    try {
      ...
      console.error(
        "Erro ao buscar dependências (cursos e disciplinas):",
        error,
      );
    }
  };
  ...
  const handleOpenEditModal = async (curriculum: Curriculum) => {
    try {
      ...
      setFormData({
        ...
        subjects:
          fullCurriculum.subjects?.map((s: CurriculumSubject) => ({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEditing ? `/curriculums/${formData.id}` : "/curriculums";
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-5">
              ...
              <div className="mt-6 border-t border-slate-100 pt-6">
                ...
                {formData.subjects.map((sub, index) => (
                  <div
                    key={index}
                    className="flex items-center gap-3 mb-3 bg-slate-50 p-3 rounded-xl"
                  >
                    <div className="flex-1">
                      <Select
                        ...
                        value={sub.subjectId}
                        onChange={(e) =>
                          updateSubject(index, "subjectId", e.target.value)
                        }
                        className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-menu-matriz outline-none text-slate-800 text-sm cursor-pointer"
                      >
                    ...
                    <div className="w-24">
                      <input
                        ...
                        placeholder="Módulo"
                        className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-menu-matriz outline-none text-slate-800 text-sm"
                        title="Módulo/Semestre"
                      />
                    </div>
                    <button
                      type="button"
