...
import { Search, Plus, Edit2, Trash2, Library, X } from 'lucide-react';
...
export const Curriculums: React.FC = () => {
  ...
  const handleDelete = async (id: string | undefined) => {
    if (!id) return;
    if (!window.confirm('Tem certeza que deseja excluir esta grade curricular?')) return;
    ...
    try {
      ...
      alert('Erro ao excluir. Verifique se existem turmas vinculadas a ela.');
    }
  };
              ...
              {isLoading ? (
                ...
                curriculums.map((curriculum) => (
                  <tr key={curriculum.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-4 px-4 text-center font-bold text-[#f37021]">{curriculum.subjects?.length || 0} UCs</td>
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-5">
              <div className="grid grid-cols-2 gap-5">
                <div className="col-span-2">
                  ...
                  <input required type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} className="w-full px-4 py-3 bg-[#f8f9fc] border-none rounded-xl focus:ring-2 focus:ring-[#004a8d] outline-none transition-all text-slate-800" placeholder="Ex: Grade 2024 - Manhã" />
                </div>
                
                <div>
                  <label className="block text-sm font-bold text-slate-700 mb-2">Curso Vinculado</label>
                  <select required value={formData.courseId} onChange={(e) => setFormData({...formData, courseId: e.target.value})} className="w-full px-4 py-3 bg-[#f8f9fc] border-none rounded-xl focus:ring-2 focus:ring-[#004a8d] outline-none transition-all text-slate-800 cursor-pointer">
                ...
                <div>
                  ...
                  <select value={formData.active ? 'true' : 'false'} onChange={(e) => setFormData({...formData, active: e.target.value === 'true'})} className="w-full px-4 py-3 bg-[#f8f9fc] border-none rounded-xl focus:ring-2 focus:ring-[#004a8d] outline-none transition-all text-slate-800 cursor-pointer">
              ...
              <div className="mt-6 border-t border-slate-100 pt-6">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <button type="button" onClick={addSubject} className="text-sm bg-emerald-50 text-emerald-600 px-3 py-1.5 rounded-lg font-bold hover:bg-emerald-100 transition-colors flex items-center gap-1">
                ...
                {formData.subjects.map((sub, index) => (
                  <div key={index} className="flex items-center gap-3 mb-3 bg-slate-50 p-3 rounded-xl">
                    <div className="flex-1">
                      <select required value={sub.subjectId} onChange={(e) => updateSubject(index, 'subjectId', e.target.value)} className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#004a8d] outline-none text-slate-800 text-sm">
                    ...
                    <div className="w-24">
                      <input required type="number" min="1" value={sub.module} onChange={(e) => updateSubject(index, 'module', parseInt(e.target.value))} placeholder="Módulo" className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-[#004a8d] outline-none text-slate-800 text-sm" title="Módulo/Semestre" />
                    </div>
