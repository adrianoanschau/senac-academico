...
export const Settings: React.FC = () => {
  ...
  const [displayName, setDisplayName] = useState(profile?.displayName || user?.user_metadata?.displayName || '');
  const [phone, setPhone] = useState(() => {
    const v = (profile?.phoneNumber || user?.user_metadata?.phoneNumber || '').replace(/\D/g, '');
    if (!v) return '';
    if (v.length > 7) return `(${v.substring(0, 2)}) ${v.substring(2, 4)} ${v.substring(4, 7)}-${v.substring(7)}`;
  ...
  const handleProfileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        alertDialog(error.response.data.message || 'Falha ao atualizar o perfil.');
      } else {
    ...
    <div className="max-w-6xl mx-auto w-full">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">
          Olá, {profile?.displayName || profile?.email || user?.email}! Gerencie suas preferências e acessos aqui.
        </p>
      </div>
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    {profile?.roles?.length ? (
                      profile.roles.map((role) => (
                        <span key={role} className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border capitalize ${roleColors[role] || roleColors.MEMBER}`}>
                          {role.toLowerCase()}
                        </span>
                      ))
                    ) : (
          ...
          {activeTab === 'admin' && (
            <div className="animate-in fade-in duration-300">
              ...
              <div className="border-2 border-dashed border-slate-200 rounded-xl bg-slate-50 flex flex-col items-center justify-center h-64 text-slate-400">
                ...
                <p className="font-medium">Tabela de Gestão de Usuários e Papéis</p>
