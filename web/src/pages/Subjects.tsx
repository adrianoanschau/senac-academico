...
export const Subjects: React.FC = () => {
  ...
  const [axisFilter, setAxisFilter] = usePersistentState(
    "subjects_axis",
    "all",
  );
  ...
  const handleDelete = async (id: string | number | undefined) => {
    ...
    if (
      !(await confirmDialog(
        "Tem certeza que deseja excluir esta unidade curricular?",
      ))
    )
      return;
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEditing ? `/subjects/${formData.id}` : "/subjects";
            ...
            <form onSubmit={handleSave} className="flex flex-col gap-5">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-[#f8f9fc] border-none rounded-xl focus:ring-2 focus:ring-menu-uc outline-none transition-all text-slate-800"
              ...
              <div className="mt-4 flex justify-end gap-3">
                ...
                <button
                  type="submit"
                  disabled={isSaving}
                  className="bg-menu-uc hover:opacity-90 disabled:opacity-70 text-white px-5 py-2.5 rounded-xl font-bold transition-colors shadow-md shadow-menu-uc/30"
                >
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-4 shadow-sm mt-4">
          ...
          <div className="flex justify-between items-center text-xs text-slate-600">
            ...
            <span className="font-bold">
              {subjects.length > 0
                ? Math.round(
                    subjects.reduce((acc, curr) => acc + (curr.hours || 0), 0) /
                      subjects.length,
                  )
                : 0}
              h
