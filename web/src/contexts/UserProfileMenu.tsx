        ...
        <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-lg border border-slate-100 py-2 z-50">
            ...
            {profile?.roles && profile.roles.length > 0 && (
              <div className="flex flex-wrap gap-1 mt-2">
                {profile.roles.map((role) => (
                  <span
                    key={role}
                    className={`px-1.5 py-0.5 rounded-full text-[9px] font-bold border capitalize ${roleColors[role] || roleColors.MEMBER}`}
                  >
                    {role.toLowerCase()}
                  </span>
                ))}
          ...
          <div className="p-2">
            <Link
              ...
              className="w-full flex items-center gap-3 px-3 py-2.5 text-sm text-slate-700 rounded-lg hover:bg-slate-100 transition-colors"
            >
              <Settings size={16} />
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-2.5 text-sm text-slate-700 rounded-lg hover:bg-rose-50 hover:text-rose-600 transition-colors"
            >
              <LogOut size={16} />
